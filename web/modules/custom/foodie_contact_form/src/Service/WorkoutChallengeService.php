...
namespace Drupal\foodie_contact_form\Service;
...
class WorkoutChallengeService {
  ...
  public function getUserSuccessStories(string $search_title = NULL) {
    ...
    $config = $this->configFactory->get('foodie_contact_form.settings');
  ...
  public function logWorkout(int $user_id, string $workout_type, int $duration, int $timestamp, int|NULL $image_fid): void {
    ...
    if ($duration > 120) {
        $this->loggerFactory->get('foodie_contact_form')->warning('User {user_id} logged a {workout_type} workout of {duration} minutes.', [
          'user_id' => $user_id,
