...
namespace Drupal\foodie_notifications\Service;
...
class NotificationService {
  ...
  public function sendNotification(UserInterface $recipient, array $story_details, string $type) {
    $module = 'foodie_notifications'; // The module providing the notification.
    ...
    if ($result['result'] !== TRUE) {
      $this->loggerFactory->get('foodie_notifications')->error('There was a problem sending the email notification to %recipient.', ['%recipient' => $recipient->getEmail()]);
    }
    else {
      $this->loggerFactory->get('foodie_notifications')->notice('Notification sent to %recipient for %node.', [
